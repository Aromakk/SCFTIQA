...
def five_point_crop(idx, d_img, s_img, config):
    ...
    d_img_org = crop_image(top, left, config.crop_size, img=d_img)
    s_img_org = crop_image(top, left, config.crop_size, img=s_img)

    return d_img_org ,s_img_org

def twenty_point_crop(idx, d_img, s_img, config):
    new_h = config.crop_size
    new_w = config.crop_size
    b, c, h, w = d_img.shape
    if idx == 0:
...
class RandCrop(object):
    ...
    def __call__(self, sample):
        ...
        ret_d_sal = d_sal[:, top: top + new_h, left: left + new_w]
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        d_sal = torch.from_numpy(d_sal).type(torch.FloatTensor)
