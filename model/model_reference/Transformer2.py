...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1, embed_dim=768, depth=16,
    ...
    def forward(self, x):
        ...
        # x_saliency_guide = x[:, 1:, :].reshape(3, 224, 224)

        # print(x_saliency_guide.shape)
        x0 = x[:, 0]
        x = self.head(x0)
        if self.training:
            return x
...
class VisionTransformer2(nn.Module):
    ...
    def __init__(self,num_patches = 490 , num_classes=1, embed_dim=192, depth=16,
                 num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, vis=False):
        super().__init__()
        self.num_classes = num_classes
...
# # input = torch.randn(1, 490, 192)
# image = torch.randn(4, 3, 224, 224)
# saliency = torch.randn(4, 1, 224, 224)
#
#
